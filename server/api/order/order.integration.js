...
describe('Order API:', function() {
  ...
  describe('GET /api/users/:userId/order', function() {
    var orders;
  ...
  describe('POST /api/users/:userId/order', function() {
    beforeEach(function(done) {
      request(app)
        .post('/api/users/:userId/order')
        .send({
          name: 'New Order',
  ...
  describe('GET /api/users/:userId/order/:id', function() {
    var order;

    beforeEach(function(done) {
      request(app)
        .get('/api/users/:userId/order/' + newOrder._id)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          if (err) {
  ...
  describe('PUT /api/users/:userId/order/:id', function() {
    var updatedOrder;

    beforeEach(function(done) {
      request(app)
        .put('/api/users/:userId/order/' + newOrder._id)
        .send({
          name: 'Updated Order',
  ...
  describe('DELETE /api/users/:userId/order/:id', function() {

    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete('/api/users/:userId/order/' + newOrder._id)
        .expect(204)
        .end((err, res) => {
          if (err) {
            return done(err);
